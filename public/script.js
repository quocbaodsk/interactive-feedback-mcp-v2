...
class FeedbackUI {
    ...
    async handleSubmitFeedback() {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                try {
                    ...
                    // If we got here, server might have sent response but connection closed
                    // Assume success if status was 200
                    result = { success: true };
                }
                ...
                if (result.success) {
                    // SUCCESS! Show messages and close
                    this.showMessage(
                        currentLanguage === 'vi' 
                    ...
                    setTimeout(() => {
                        ...
                        // If still here, show success screen
                        setTimeout(() => {
        ...
        // Check if feedback might have been saved anyway
        this.showMessage(
            currentLanguage === 'vi'
                ? '⚠️ Không nhận được xác nhận từ server. Feedback có thể đã được gửi. Bạn có thể đóng cửa sổ và kiểm tra với AI.'
                : '⚠️ No confirmation from server. Feedback may have been sent. You can close this window and check with AI.',
            'error'
        );
    ...
    /**
     * Show success screen with close button
     */
    showSuccessScreen() {
        document.body.innerHTML = `
            ...
            ">
                ...
                <h1 style="color: #58a6ff; margin-bottom: 10px;">
                    ${currentLanguage === 'vi' ? 'Hoàn tất!' : 'Done!'}
                </h1>
                <p style="color: #c9d1d9; font-size: 18px; margin-bottom: 20px;">
                    ${currentLanguage === 'vi' 
                ...
                <p style="color: #8b949e; font-size: 14px; margin-bottom: 30px;">
                    ${currentLanguage === 'vi'
                        ? 'Bạn có thể đóng cửa sổ này và quay lại chat với AI.'
                        : 'You can close this window and return to AI chat.'}
                </p>
                ...
                " onmouseover="this.style.background='#2ea043'" onmouseout="this.style.background='#238636'">
                    ${currentLanguage === 'vi' ? '✕ Đóng cửa sổ' : '✕ Close Window'}
                </button>
            </div>
        `;
    }
    ...
    setFormDisabled(disabled) {
        ...
        if (this.elements.submitFeedbackBtn) {
            ...
            if (disabled) {
                this.elements.submitFeedbackBtn.textContent = 
                    currentLanguage === 'vi' ? 'Đang gửi...' : 'Submitting...';
            } else {
                this.elements.submitFeedbackBtn.textContent = 
                    currentLanguage === 'vi' ? 'Gửi Phản hồi' : 'Submit Feedback';
            }
        }
    }
